...
Launches an application via `Process.Start` with `UseShellExecute = true`. By default, the companion does not call `SetForegroundWindow`, `ShowWindow`, or any other foreground-forcing API after launch; foreground ownership is left to Windows, Game Bar, and the launched application.

**Note:** The widget prefers `XboxGameBarWidget.LaunchUriAsync` for URL and Store launches so Game Bar owns overlay dismissal. The `launch` IPC action is normally the fallback for EXE items with arguments or when `LaunchUriAsync` is unavailable. Because `focusLaunchedApps` defaults to enabled, EXE launches use this action by default so the companion can foreground the launched process window when it can identify one. URL and Store launches continue to prefer `LaunchUriAsync`.
...
| `focusLaunchedApp` | string | no | `"true"` to foreground the launched app when possible. Defaults to `"false"` if omitted from the raw IPC request; the widget sends `"true"` by default through config. |
| `launchId` | string | no | Correlation id for widget and companion launch/focus log lines. Generated by the companion if omitted. |
| `focusDelayMs` | string | no | Delay before foreground focus is attempted. Used to let Game Bar finish overlay dismissal before focus APIs run. |
...
| `launchId` | string | always | Correlation id used in logs |
...
ValueSet {
    ...
    ["args"]   = "--fullscreen",
    ["focusLaunchedApp"] = "false",
    ["launchId"] = "7ca4616fec6b457a84b6a036ed760cb8",
    ["focusDelayMs"] = "350"
}
```
...
```
ValueSet { ["status"] = "ok", ["launchId"] = "7ca4616fec6b457a84b6a036ed760cb8" }
```

**Example error response:**

```
ValueSet { ["status"] = "error", ["launchId"] = "7ca4616fec6b457a84b6a036ed760cb8", ["error"] = "The system cannot find the file specified." }
```

**Widget client:** `CompanionClient.LaunchAsync(string type, string path, string? args, bool focusLaunchedApp, string? launchId, int focusDelayMs)` returns `bool` (`true` on success).
